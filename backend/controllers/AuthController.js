...
export const SignupUser = async (req, res) => {
    try { // ~ Validation Errors ~
        ...
        if (isEmail) {
            return res.status(400).json({success: false, message: "Email already ragistered try another"});
        }
...
export const VerifyUser = async (req, res) => {
    try {
        ...
        if (! verificationToken) {
            return res.status(403).json({success: false, message: "Verification Token is required"});
        }
        const user = await UserModel.findOne({verificationToken: verificationToken});

        if (! user) {
            return res.status(403).json({success: false, message: " Verification Issue: The provided email was not found or has already been verified."});
        }
        ...
        res.status(504).json({succecc: false, message: error.message});
    }
};
...
export const deleteUser = async (req, res) => {
    try {
        ...
        if (isDeleteUser) {
            return res.status(200).json({success: true, message: "The User Deleted Succefully"});
        } else {
            return res.status(400).json({success: false, message: "The Not User Deleted..."});
        }
    } catch (error) {
        console.log(error);
...
export const LoginUser = async (req, res) => {
    try {
        ...
        if (! user) {
            return res.status(400).json({success: false, message: "Email are not ragistered , ragister now "});
        }
        if (user.isGoogleLogin) {
            return res.status(400).json({success: false, message: "Already Login using the google login option..."});
        }
        if (! user.verified) {
            return res.status(400).json({success: false, message: "User are not verified"});
        }

        const isPassword = await user.isPasswordValid(password);
        if (! isPassword) {
            return res.status(400).json({sucess: false, message: "Login credintials invalid"});
        }
...
export const ForgotPassword = async (req, res) => {
    try {
        ...
        return res.status(200).json({success: true, message: "OTP sent to your email succefully", email: email});
    } catch (error) {
        console.log(error);
        res.status(504).json({sucess: false, message: error.message});
    }
};
...
export const OTP_Verification = async (req, res) => {
    try {
        const {otp} = req.body;
        if (!otp) {
            return res.status(400).json({sucess: false, message: "OTP is required"});
        }

        if (otp.toString().length !== 6) {
            return res.status(400).json({sucess: false, message: "OTP length must be at least 6 digit"});
        }
        ...
        if (user) {
            ...
            return res.status(200).json({success: true, message: "OTP Verification Done", email: user.email});
        }
    } catch (error) {
        console.log(error);
        res.status(504).json({sucess: false, message: error.message});
...
export const ResetPassword = async (req, res) => {
    try {
        ...
        const errros = validationResult(req);
        if (! errros.isEmpty()) {
            return res.status(400).json({success: false, message: errros.array()});
        }

        // ~~ Check Email ~~
        const user = await UserModel.findOne({email: email});
        if (! user) {
            return res.status(400).json({success: false, message: "Sorrry Somenthing Went Wrong..."});
        }

        const hashedPassword = await bcrypt.hash(password, 10);
        const isUpdated = await UserModel.findByIdAndUpdate(user._id, {password: hashedPassword});

        if (isUpdated) {
            return res.status(200).json({success: true, message: "Password reset succefully"});
        }
    } catch (error) {
        console.log(error);
        res.status(504).json({success: false, message: error.message});
...
export const ResendOtp = async (req, res) => {
    try {
        ...
        return res.status(200).json({success: true, message: "OTP resent to your email succefully"});
    } catch (error) {
        console.log(error);
        res.status(504).json({success: false, message: error.message});
    }
...
export const GoogleLogin = async (req, res) => {
    try {
        ...
        if (user && user.isGoogleLogin === false) {
            return res.status(400).json({success: false, message: "User already exists. Please log in using email and password."});
        }
        ...
        if (! user) {
            ...
            const token = user.generateAuthToken();
            return res.status(200).json({success: true, message: "User Login Successfull", user: user, token: token});
        }
        if (user.isGoogleLogin) {
            const token = user.generateAuthToken();
            return res.status(200).json({success: true, message: "User Login Successfull", user: user, token: token});
        ...
        console.log(error);
        return res.status(504).json({sucess: false, message: error.message});
    }
};
...
export const PrivateAuth = async (req, res) => {
    const user = req.user;
    return res.status(200).json({success: true, message: "Authentication Succefull", isValid: true});
};
...
export const Logout = async (req, res) => {
    try {
        ...
        await BlackListedToken({token: token, expiresAt: expires});
