...
import { encryptData } from "../../component/features/Hashing";
const SignupPage = () => {
  ...
  const SignupFormik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (SignupAsynkThunk.fulfilled.match(result)) {
        ...
        setisLoading(false);
        const encryptSignup = encryptData(true);
        localStorage.setItem("isSignup", encryptSignup);
        navigate(`/auth/email-verification?email=${result.payload.user.email}`);
      } else {
        toast.update(toastId, {
      ...
      <div className="w-full md:w-1/2 bg-white flex flex-col justify-between px-6 py-8 md:px-16 md:py-12">
        <div className="max-w-md mx-auto flex-1 flex flex-col justify-center">
          ...
          >
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  name="remember"
                  value={SignupFormik.values.remember}
                  onChange={SignupFormik.handleChange}
                  className="w-3 h-3"
                />
            ...
            <button
              type="submit"
              className="w-full h-10 bg-[#2f46f0] text-white text-sm font-semibold flex items-center justify-center hover:bg-[#2439c0] transition rounded"
            >
              {isLoading ? (
